...
\abstract
This thesis investigates the integration of financial news sentiment into stock market forecasting for the Malaysian equity market. The research explores how language models FinBERT and FinGPT can extract actionable insights from financial and political news to enhance the accuracy of stock price prediction models. The motivation stems from the observation that Malaysian stock prices are often influenced by local and regional news, yet existing predictive systems rarely account for this contextual sentiment. This study addresses the challenges of adapting sentiment models trained primarily on Western data to a Malaysian linguistic and market context and of integrating qualitative news data with quantitative time-series market data. 

The research consists of three empirical studies.
\begin{enumerate}
    \item \textbf{Sentiment Model Benchmarking}: The first investigation evaluates FinBERT and FinGPT on Malaysian financial and political news headlines, assessing both zero-shot and fine-tuned performance. The analysis benchmarks sentiment classification accuracy using precision, recall, and F1-scores, and compares the contextual relevance of sentiment polarity across models. 
    
    \item \textbf{Sentiment vs. Stock Movement}: The second investigation investigates the correlation between daily sentiment scores and Kuala Lumpur Stock Exchange (KLSE) stock returns and volatility across multiple time lags (T, T+1, T+2). Regression, correlation, and Granger causality tests are applied to identify the temporal influence of sentiment on market behaviour.
    
    \item \textbf{Stock Price Prediction Models}: The final investigation develops predictive models that combine engineered stock features such as moving averages, RSI, MACD, and volatility with sentiment scores derived from FinBERT and FinGPT. LSTM, GRU, and transformer-based models are trained and benchmarked using RMSE, MAE, and directional accuracy metrics.
\end{enumerate}


The thesis presents following contribution to science:
\begin{enumerate}
    \item \textbf{First Benchmarking of FinBERT vs FinGPT on Malaysian News}. This research presents the first comparative analysis between FinBERT and FinGPT for sentiment classification in Malaysian financial and political contexts. It contributes new evidence on model generalization and domain transferability of large language models (LLMS) trained on Western data when applied to emerging markets
    ...
    \item \textbf{Application of Transformer-based Models in Emerging Markets}. By demonstrating the potential of FinGPT-enhanced transformer models in forecasting market trends within non-western setting, this research expands the application of state-of-the-art NLP-driven finance models to emerging markets such as Malaysia.
...
\chapterabstract{%
This chapter introduces the research problem of enhancing financial forecasting in the Malaysian stock market (KLSE) by integrating unstructured news sentiment with deep learning models. It outlines the motivation for moving beyond traditional technical analysis to adopt Large Language Models (LLMs), specifically comparing the discriminative FinBERT and generative FinGPT architectures. The chapter details the research objectives and provides a high-level overview of the three core experiments: sentiment model benchmarking, statistical correlation analysis, and hybrid stock price prediction. Finally, it highlights the study's contributions to science—specifically the application of Transformer-based models in emerging markets—and concludes with an outline of the thesis structure.
}
...
\begin{enumerate}
    ...
    This objective involves fine-tuning FinBERT, a discriminative encoder-based model, and instruction-tuning FinGPT, a generative decoder-based model, using a curated dataset of Malaysian financial news. The aim is to determine which architectural paradigm more effectively captures local linguistic nuances, financial jargon, and the pronounced neutral-class imbalance characteristic of Malaysian corporate reporting.

    \item \textbf{Quantify the relationship between extracted sentiment signals and KLSE market volatility.}  
    This objective seeks to statistically analyse the correlation between sentiment scores produced by both models and observed price movements of KLSE-listed stocks. In particular, it evaluates whether the enhanced numerical sensitivity and contextual reasoning of generative models enable more effective detection of market-moving information and event-driven shocks compared to static, pattern-based sentiment classification.
...
This thesis adopts a progressive empirical design that begins with an evaluation of the linguistic capabilities of domain-specific sentiment models, advances to a statistical analysis of the relationship between sentiment signals and market volatility, and culminates in an out-of-sample forecasting study. The research is structured into three interconnected empirical experiments, each addressing a distinct aspect of the proposed research objectives.

\begin{enumerate}
    \item \textbf{Benchmarking Discriminative versus Generative Sentiment Models on Malaysian Financial Corpora}  
    The first experiment seeks to empirically characterise the trade-off between \emph{classification accuracy} and \emph{contextual reasoning} within the Malaysian financial domain. FinBERT, a discriminative encoder-based model, has established state-of-the-art performance in financial sentiment classification through supervised fine-tuning; however, it typically requires large volumes of labelled data and often struggles with implied or context-dependent sentiment commonly present in complex financial narratives. In contrast, FinGPT, a generative decoder-based model, leverages instruction tuning to reformulate sentiment analysis as a text generation task.  
    This experiment benchmarks both architectures using a curated dataset of Malaysian financial news sourced from outlets such as \emph{The Star} and \emph{New Straits Times}, with particular emphasis on their ability to handle neutral-class imbalance and local economic terminology that generic models frequently misclassify.

    \item \textbf{Statistical Analysis of Market Sensitivity to Sentiment Shocks}  
    The second experiment examines the extent to which sentiment signals extracted from financial news are associated with volatility in the Bursa Malaysia (KLSE). Moving beyond classification metrics, this study evaluates whether the sentiment scores generated in Experiment~1 exhibit statistically meaningful relationships with observed stock price movements. Drawing on established literature linking news sentiment to financial volatility, the analysis employs statistical techniques such as correlation analysis and Granger causality tests to quantify the lagged impact of news on selected KLSE blue-chip stocks, including Maybank and Genting Berhad. The objective is to determine whether the generative reasoning capabilities of FinGPT capture event-driven market shocks more effectively than the static sentiment probabilities produced by FinBERT.

    \item \textbf{Hybrid Forecasting with Generative Augmentation: FinBERT--LSTM versus FinGPT--LSTM}  
    The final experiment integrates insights from the preceding studies to develop and evaluate hybrid forecasting architectures. Traditional econometric models and standalone Long Short-Term Memory (LSTM) networks often struggle to capture non-linear dependencies induced by qualitative market events. In this experiment, sentiment representations are fused with historical OHLCV (Open, High, Low, Close, Volume) data to form enriched input sequences for time-series prediction. A sliding window validation strategy is employed to prevent look-ahead bias. The predictive performance of a baseline LSTM model is compared against hybrid models incorporating sentiment features from FinBERT and FinGPT. Model performance is assessed using Root Mean Square Error (RMSE) and Mean Absolute Percentage Error (MAPE), with the primary objective of determining whether the additional computational cost of Generative AI yields a statistically significant improvement in forecasting accuracy over lightweight discriminative baselines in the Malaysian market context.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{First Comparative Benchmarking of FinBERT and FinGPT in the Malaysian Market}  
    To the best of the author’s knowledge, this study presents the first empirical comparison between discriminative (encoder-only) and generative (decoder-only) financial language models within the context of Bursa Malaysia (KLSE). While FinBERT has been extensively evaluated on developed markets such as the United States and China, and FinGPT has recently been proposed as an open-source generative alternative for financial Large Language Models (LLMs), their relative effectiveness in handling Malaysian-specific linguistic patterns, economic narratives, and pronounced neutral-class imbalance has not previously been examined. This thesis addresses this geographic and domain-specific gap.

    \item \textbf{Empirical Evaluation of Numerical Sensitivity in Financial Sentiment Analysis}  
    This research advances the methodological understanding of how Artificial Intelligence systems interpret quantitative information embedded in financial text. Conventional discriminative models such as FinBERT treat numerical values as generic tokens, often failing to account for the magnitude of financial changes. By implementing instruction tuning with FinGPT, this study empirically evaluates the model’s numerical sensitivity—its ability to reason about the scale and impact of numerical disclosures (e.g.\ a 5\% versus 50\% profit decline). The results provide novel evidence on whether generative reasoning yields a statistically significant advantage over static embedding-based approaches in financial risk interpretation.
    ...
    Finally, this study offers a practical evaluation of the trade-off between model complexity and predictive performance in financial applications. By contrasting the lightweight and efficient inference of FinBERT with the higher computational cost and latency associated with FinGPT, the research provides actionable insights into whether the deployment of generative models is justified for real-time or near–real-time trading environments in markets with lower liquidity than major global exchanges.
\end{enumerate}
...
\begin{itemize}
    ...
    \item Chapter 3 - \textbf{Sentiment Model Benchmarking}. This chapter provides an empirical benchmarking analysis to compare the capabilities of FinBERT and FinGPT in extracting sentiment from Malaysian financial and political news. The study investigates the efficacy of zero-shot learning versus fine-tuning on a curated, localized dataset. This experiment is conducted to determine which model better captures the contextual nuances of the Malaysian market prior to integration into predictive models.

    \item Chapter 4 - \textbf{Sentiment vs. Stock Movement}. This second study explores the correlation between the daily sentiment scores derived in Chapter 3 and the actual market movements of top Malaysian companies. The analysis employs lag analysis (T, T+1, T+2) and statistical tests (Pearson/Spearman correlation, Granger Causality) to investigate whether news sentiment serves as a leading indicator for stock returns and volatility in the KLSE.

    \item Chapter 5 - \textbf{Stock Price Prediction Models}. This study integrates the findings of the previous experiments to build a hybrid predictive framework. It proposes time-series prediction models that fuse historical stock data with the engineered sentiment features. The chapter compares baseline models (LSTM/GRU with price features only) against sentiment-enhanced models (augmented with FinBERT or FinGPT scores) to quantify the value-add of news-driven insights on predictive accuracy (RMSE, MAE).
...
In contrast to FinBERT, FinGPT is built upon a decoder-only Transformer architecture, typically derived from LLaMA-based models, and is designed for autoregressive text generation. To enable this generative model to perform sentiment classification, this thesis employs \emph{instruction tuning}.
...
\paragraph{Zero-Shot vs.\ Few-Shot Learning}

This research evaluates two prompting strategies to assess the model’s baseline generalisation capability and its adaptability to the Malaysian market context:

\begin{itemize}
    \item \textbf{Zero-Shot Learning:}  
    In the zero-shot setting, the model is provided only with a task description and a financial news headline, without any labelled examples. The prompt relies entirely on the model’s pre-trained knowledge to infer sentiment (e.g.\ \emph{``Classify the sentiment of the following news:''}). This configuration evaluates the model’s intrinsic understanding of generic financial concepts.

    \item \textbf{Few-Shot Learning (In-Context Learning):}  
    To capture linguistic, political, and economic nuances specific to Bursa Malaysia, a small number of labelled demonstrations ($k$-shot examples) are included within the prompt context window prior to the query. For example, the prompt may contain three Malaysian financial news headlines labelled as Positive, Negative, and Neutral. This in-context learning mechanism enables the model to infer local sentiment conventions—such as the positive market implications of political stability in Malaysia—without requiring gradient-based parameter updates.
\end{itemize}
...
\chapterabstract{This chapter presents a comparative empirical study of FinBERT and FinGPT to evaluate their efficacy in extracting financial sentiment within the Malaysian market context. While FinBERT has demonstrated state-of-the-art performance in standard financial sentiment classification tasks, the emergence of open-source Large Language Models (LLMs) like FinGPT offers potential improvements in handling complex, contextual narratives through instruction tuning. This study benchmarks these architectures on a curated dataset of local news headlines, critically analyzing the trade-offs between the computational efficiency of encoder-only models and the contextual adaptability of generative models. The findings identify the optimal architecture for capturing the nuances of Malaysian financial reporting, serving as the foundational sentiment signal for the downstream stock prediction experiments in subsequent chapters.}
...
\chapterabstract{This chapter empirically investigates the statistical relationship between the aggregated daily sentiment scores derived in Chapter 3 and the market returns of top constituents of the Bursa Malaysia (KLSE). By employing lag analysis (T+1, T+2) and Granger Causality tests, the study examines whether news sentiment functions as a leading indicator for local market volatility or merely reflects past price actions. The analysis critically evaluates the temporal horizon at which sentiment signals exhibit the strongest correlation with market directionality.}
...
\chapterabstract{This study develops and evaluates a hybrid predictive framework by fusing historical market data (OHLCV) with the sentiment features extracted via FinBERT and FinGPT. Addressing the limitations of traditional technical analysis, the chapter constructs Long Short-Term Memory (LSTM) and Gated Recurrent Unit (GRU) networks to forecast short-term stock prices. The experiment benchmarks the predictive accuracy (RMSE, MAE, MAPE) of sentiment-enhanced models against baseline models relying solely on price history, thereby quantifying the specific value-add of generative AI-driven insights in the Malaysian financial context.}
...
\chapterabstract{The final chapter synthesizes the empirical findings regarding the comparative efficacy of encoder-only (FinBERT) and decoder-based (FinGPT) Large Language Models in financial forecasting. It summarizes the research contributions, specifically the validation of localized sentiment as a predictive feature for the KLSE. The thesis concludes by identifying limitations in current methodologies and proposing future research directions, including the development of real-time sentiment dashboards and the integration of multilingual capabilities (Malay/Chinese) to capture the full spectrum of local investor sentiment.}
