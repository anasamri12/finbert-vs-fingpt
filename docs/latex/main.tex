...
A 100-article benchmark was constructed from Malay-language financial news articles, manually labelled with positive, neutral, and negative sentiment. Six model configurations were evaluated: zero-shot TinyLlama-1.1B, zero-shot Llama-3.2-3B, a LoRA fine-tuned variant of each, and two FinBERT variants (domain-pretrained and fine-tuned on pseudo-labelled in-domain data). Fine-tuned FinBERT achieved the highest macro F1 of 0.620, confirming that a compact discriminative encoder, properly specialised to the financial domain, outperforms zero-shot generative models on this task. Llama-3.2-3B zero-shot attained macro F1 of 0.535, substantially improving on TinyLlama (F1=0.326) and demonstrating that general reasoning capability matters even without domain fine-tuning. Crucially, LoRA fine-tuning \emph{degraded} performance for both LLMs, with TinyLlama-LoRA falling to F1=0.162. This is consistent with catastrophic forgetting in small-capacity networks when instruction-tuned on limited labelled data ($n=100$). Pseudo-label adaptation of FinBERT yielded a marginal gain over the base domain-pretrained model (+0.010 macro F1), insufficient to close the gap to supervised fine-tuning on ground-truth labels. The neutral class remained the hardest to classify across all models (recall as low as 0.28), reflecting a genuine ambiguity in financial language rather than a modelling failure.
...
Price-only LSTM and GRU models outperformed persistence on directional accuracy for Maybank ($\approx$63\,\%) and showed modest R² improvements for Maybank (R²=0.038--0.057) and Tenaga (after re-run at horizon=1). For CIMB and RHB, price-only models failed to beat persistence reliably. Sentiment augmentation produced the clearest benefit for RHB, where GRU directional accuracy rose from 52.6\,\% to 60.0\,\% (+7.4 pp) and LSTM rose to 61.1\,\% (+8.5 pp)---the largest sentiment lift observed across the study. For Maybank, LSTM directional accuracy improved slightly with sentiment features (58.4\,\%~$\to$~61.1\,\%). For CIMB and Tenaga, sentiment augmentation produced no consistent directional improvement, consistent with the Granger bidirectionality finding for CIMB and the weak lag-1 signal for Tenaga. Overall, the models confirm that sentiment features carry directional information not captured by price history alone for certain stocks, while corroborating the EMH view that this information is modest in magnitude and stock-specific.
...
\begin{enumerate}
    ...
    \item \textbf{Systematic comparison of FinBERT and generative LLMs under Malaysian market conditions.} The benchmark evaluation demonstrates that fine-tuned FinBERT (macro F1=0.620) outperforms zero-shot Llama-3.2-3B (F1=0.535) and TinyLlama-1.1B (F1=0.326) on this task, and that LoRA fine-tuning with small labelled datasets actively harms performance through catastrophic forgetting. This provides practitioners with concrete, empirically grounded guidance for model selection.
...
Llama-3.2-3B zero-shot already shows promise relative to TinyLlama. Evaluating larger open models (e.g., Llama-3-8B, Mistral-7B, Qwen-7B) in both zero-shot and instruction-tuned configurations would clarify the scaling behaviour of generative models on Malay financial sentiment. In particular, models pre-trained on more diverse multilingual data may achieve competitive macro F1 without any domain fine-tuning, which would be practically valuable given the difficulty of constructing large labelled corpora for low-resource financial languages.
