...
These weak broad-corpus results motivated the company-filtered follow-up. Once the news corpus was restricted to company-relevant articles, the same-day relationship became materially stronger for several stocks. Maybank, CIMB, and Tenaga all exhibited moderate positive lag-0 correlation and directional accuracy above 0.64, while RHB improved only modestly. The filtered corpora also provided adequate daily article volume for stable aggregation: the mean number of matched articles per market day was 7.34 for Maybank, 7.01 for CIMB, 5.86 for Tenaga, and 4.57 for RHB. Table~\ref{tab:experiment2-filtered-summary} summarises the contrast between the broad KLSE result and the strongest filtered stock-level results. Table~\ref{tab:experiment2-filtered-lag} presents the full lag profile for each filtered stock.
...
Five implications follow from Tables~\ref{tab:experiment2-filtered-summary} and~\ref{tab:experiment2-filtered-lag}. First, the weak KLSE index result is not sufficient evidence that sentiment is useless; rather, it suggests that broad aggregation obscures stock-relevant information. Second, company-specific filtering substantially strengthens the relationship for selected firms, especially Maybank, CIMB, and Tenaga, and these lag-0 effects are highly statistically significant. For Maybank, the lag-0 Pearson correlation of 0.320 had $p = 6.59 \times 10^{-19}$, with an OLS coefficient of 0.00620 and $R^2 = 0.103$. For CIMB, the lag-0 Pearson correlation of 0.342 had $p = 1.92 \times 10^{-21}$, with an OLS coefficient of 0.0110 and $R^2 = 0.117$. For Tenaga, the lag-0 Pearson correlation of 0.337 had $p = 1.51 \times 10^{-20}$, with an OLS coefficient of 0.00878 and $R^2 = 0.114$. Third, the effect is predominantly contemporaneous rather than predictive: for Maybank, Pearson correlation fell from 0.320 at $T$ to $-0.024$ at $T{+}1$ and near-zero at $T{+}2$, and similar attenuation occurred for CIMB. This pattern is consistent with the semi-strong form of the Efficient Market Hypothesis (EMH), which predicts that publicly available news is rapidly incorporated into asset prices, leaving little exploitable lead--lag relationship. Fourth, Granger evidence is limited and stock-specific. Maybank showed no significant Granger causality in either direction. CIMB exhibited bidirectional significance at lag~1, suggesting association but not clean one-way predictiveness. Tenaga produced the clearest forward-looking result, with sentiment Granger-causing returns at lag~3 ($p = 0.014$) while the reverse direction remained non-significant. RHB, by contrast, showed no significant lag-0 association ($p = 0.081$) and only reverse Granger significance from returns to sentiment at lag~2 ($p = 0.044$), suggesting a more reactive than predictive signal. Fifth, the weaker result for RHB is plausibly related to data quality and coverage: it had the smallest article flow among the four main filtered stocks, and the directional coverage column confirms that approximately 11--14\% of trading days had no matched articles, meaning the sentiment signal is absent for a non-trivial fraction of the evaluation period.

\section{Summary}
This chapter establishes five main findings. First, broad market-wide sentiment derived from the full 2023--2025 corpus has negligible association with KLSE index returns, though this result is based on a limited number of matched trading days and should be interpreted cautiously. Second, company-specific filtering is crucial: once irrelevant articles are removed, FinBERT sentiment exhibits statistically significant same-day relationships for selected firms, most notably Maybank, CIMB, and Tenaga, with Pearson correlations of 0.32--0.34, p-values far below 0.001, and directional accuracy above 0.64. Third, simple univariate OLS models confirm that the lag-0 effects are economically meaningful but still partial, with $R^2$ values around 0.10--0.12 for Maybank, CIMB, and Tenaga. Fourth, the sentiment signal drops sharply beyond lag~0 for most stocks, making it better interpreted as a same-day market-association measure than as a broad next-day forecasting feature; the clearest exception is Tenaga, which shows one-way Granger significance from sentiment to returns at lag~3. Fifth, the signal is not uniform across all firms: RHB's weaker and mostly non-significant result highlights that corpus size and article relevance quality materially affect downstream performance. These findings motivate the use of relevance-aware, company-filtered sentiment features in the forecasting experiments of Chapter~5, rather than a single broad market-wide aggregate.
