...
\begin{enumerate}
    ...
    This objective seeks to statistically analyse the correlation between sentiment scores produced by the strongest benchmarked models and observed price movements of KLSE-listed stocks. In particular, it evaluates whether richer contextual modelling from generative backbones or stronger encoder-based classification from FinBERT produces more useful signals for detecting market-moving information and event-driven shocks.
...
\section{Results}
The zero-shot comparison showed that larger open generative backbones substantially outperformed the original TinyLlama baseline. Among the zero-shot generative models, Meta-Llama-3.1-8B-Instruct achieved the strongest result on the 2025 development set, followed by Qwen2.5-7B-Instruct. Nevertheless, supervised FinBERT remained clearly superior overall.
...
\hline
\textbf{Stage} & \textbf{Model} & \textbf{Accuracy} & \textbf{Macro F1} \\
\hline
Zero-shot & ProsusAI/finbert & 0.25 & 0.250 \\
Zero-shot & TinyLlama-1.1B-Chat-v1.0 & 0.25 & 0.133 \\
Zero-shot & Llama-3.2-1B-Instruct & 0.33 & 0.305 \\
Zero-shot & Qwen2.5-3B-Instruct & 0.42 & 0.377 \\
Zero-shot & Llama-3.2-3B-Instruct & 0.43 & 0.385 \\
Zero-shot & Qwen2.5-7B-Instruct & 0.46 & 0.410 \\
Zero-shot & Meta-Llama-3.1-8B-Instruct & 0.47 & 0.458 \\
Supervised & TinyLlama-based FinGPT run2 & 0.35 & 0.328 \\
Supervised & Qwen2.5-3B-Instruct + LoRA & 0.42 & 0.394 \\
Supervised & FinBERT run2 & 0.69 & 0.620 \\
Adapted & TinyLlama-based FinGPT adapted & 0.33 & 0.297 \\
Adapted & FinBERT adapted & 0.66 & 0.546 \\
\hline
\end{tabular}
\caption{Summary of key sentiment benchmarking results on the 2025 human-labeled development set. Macro F1-score is the primary model-selection metric.}
\label{tab:sentiment-benchmark-2025}
\end{table}

Several patterns are evident from Table~\ref{tab:sentiment-benchmark-2025}. First, the poor early performance of the TinyLlama-based FinGPT baseline was not solely a consequence of the generative formulation itself; replacing the backbone with larger Qwen or Llama instruct models produced substantial zero-shot gains. Second, a modest supervised Qwen-3B LoRA run improved on the corresponding zero-shot Qwen-3B baseline, indicating that the 60k corpus remains useful for generative adaptation. Third, despite these improvements, the best generative zero-shot model (Meta-Llama-3.1-8B-Instruct, macro F1 $=0.458$) still trailed the 60k-supervised FinBERT model by a large margin.

The pseudo-label adaptation experiments produced a negative but informative result. FinBERT adaptation on the recent 2023--2025 pseudo-labeled corpus reduced macro F1 from 0.620 to 0.546 on the 2025 development set, while TinyLlama-based FinGPT adaptation reduced macro F1 from 0.328 to 0.297. This suggests that, in the current setup, pseudo-label noise outweighed any benefit from exposure to more recent unlabeled Malaysian news. Consequently, the 60k-supervised FinBERT model was selected as the main downstream sentiment model for the later forecasting experiments.

\section{Summary}
This chapter establishes three main findings. First, supervised FinBERT is the strongest sentiment model in the current Malaysian benchmark and therefore the most appropriate core signal for downstream forecasting. Second, larger open instruct backbones such as Qwen2.5-7B and Meta-Llama-3.1-8B substantially improve the zero-shot generative baseline relative to TinyLlama, indicating that backbone capacity is a major factor in generative financial sentiment transfer. Third, pseudo-label adaptation on recent unlabeled news did not improve performance in this study. The remainder of the thesis therefore treats the 60k-supervised FinBERT model as the primary sentiment extractor, while the stronger open generative baselines serve as comparative evidence on the trade-off between accuracy and computational cost.
