...
\section{Results}
The expanded benchmark sharpened the contrast between discriminative and generative sentiment extraction. Among the zero-shot generative models, Meta-Llama-3.1-8B-Instruct achieved the strongest result on the 2025 development set, followed by Qwen2.5-7B-Instruct. However, supervised FinBERT remained clearly superior overall and was the only model family to retain strong performance on the held-out 2026 evaluation.
...
Supervised & Qwen2.5-7B-Instruct + LoRA (5e-5) & 0.42 & 0.343 \\
Supervised & Qwen2.5-7B-Instruct + LoRA (1e-5) & 0.42 & 0.357 \\
Supervised & Meta-Llama-3.1-8B-Instruct + LoRA & 0.44 & 0.431 \\
Supervised & Public FinGPT Llama2-7B & 0.27 & 0.175 \\
Supervised & Public FinGPT Llama2-13B & 0.25 & 0.133 \\
...
Several patterns are evident from Table~\ref{tab:sentiment-benchmark-2025}. First, the weak early performance of the TinyLlama-based FinGPT baseline was not solely a consequence of the generative formulation itself; replacing the backbone with larger Qwen or Llama instruct models produced substantial zero-shot gains. Second, not all supervised generative adaptation transferred equally well. Qwen-3B improved slightly after LoRA tuning (macro F1 0.394 versus 0.377 zero-shot), but both Qwen-7B LoRA runs remained below the Qwen-7B zero-shot baseline, and the fine-tuned Llama-3.1-8B result (macro F1 0.431) also remained below its own zero-shot baseline of 0.458. Third, the official public-source FinGPT checkpoints transferred poorly to the Malaysian benchmark, with macro F1-scores of only 0.175 for the Llama2-7B multi-task adapter and 0.133 for the Llama2-13B sentiment adapter.

The pseudo-label adaptation experiments produced a negative but informative result. FinBERT adaptation on the recent 2023--2025 pseudo-labeled corpus reduced macro F1 from 0.620 to 0.546 on the 2025 development set, while TinyLlama-based FinGPT adaptation reduced macro F1 from 0.328 to 0.297. This suggests that, in the current setup, pseudo-label noise outweighed any benefit from exposure to more recent unlabeled Malaysian news. For completeness, the selected 60k-supervised FinBERT model was also evaluated on the held-out 2026 benchmark, where it achieved 0.69 accuracy and macro F1 0.598, confirming that the model selection made on the 2025 development set generalized reasonably well.

\section{Summary}
This chapter establishes four main findings. First, supervised FinBERT is the strongest sentiment model in the current Malaysian benchmark and therefore the most appropriate core signal for downstream forecasting. Second, larger open instruct backbones such as Qwen2.5-7B and Meta-Llama-3.1-8B substantially improve the generative zero-shot baseline relative to TinyLlama, indicating that backbone capacity is a major factor in generative financial sentiment transfer. Third, supervised LoRA tuning of the larger decoder models does not consistently improve transfer to the human-labeled Malaysian benchmark, and the public-source FinGPT checkpoints perform especially weakly. Fourth, pseudo-label adaptation on recent unlabeled news does not improve performance in this study. The remainder of the thesis therefore treats the 60k-supervised FinBERT model as the primary sentiment extractor, while the stronger open generative baselines serve as comparative evidence on the trade-off between accuracy and computational cost.
...
\hline
\end{tabular}
\caption{Comparison between broad-corpus KLSE results and company-filtered stock-level results in Experiment~2.}
\label{tab:experiment2-filtered-summary}
\end{table}

Three implications follow from Table~\ref{tab:experiment2-filtered-summary}. First, the weak KLSE index result is not sufficient evidence that sentiment is useless; rather, it suggests that broad aggregation obscures stock-relevant information. Second, company-specific filtering substantially strengthens the relationship for selected firms, especially Maybank, CIMB, and Tenaga. Third, the effect is mainly contemporaneous rather than predictive: for the filtered stocks, the strongest results occurred at lag 0, whereas most $T{+}1$ and $T{+}2$ correlations remained weak. In other words, the filtered sentiment signal is better interpreted as a same-day market-association measure than as a robust next-day forecasting feature.
\section{Summary}
This chapter establishes three main findings. First, broad market-wide sentiment derived from the full 2023--2025 corpus has negligible association with KLSE index returns and only weak stock-level signal. Second, company-specific filtering is crucial: once irrelevant articles are removed, FinBERT sentiment exhibits materially stronger same-day relationships for selected firms, most notably Maybank, CIMB, and Tenaga. Third, the sentiment signal is not universally strong across all firms, as shown by the weaker RHB result, and it remains much stronger at lag 0 than at $T{+}1$ or $T{+}2$. These findings motivate the use of relevance-aware sentiment features in later forecasting experiments rather than a single broad market-wide aggregate.
