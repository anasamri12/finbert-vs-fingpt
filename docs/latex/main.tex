...
Encoder-only transformers remain strong baselines for sentence-level financial sentiment classification. FinBERT adapted BERT to financial text and showed that domain-specialised pre-training materially improves sentiment classification over generic language models.\footnote{Dogu Araci, ``FinBERT: Financial Sentiment Analysis with Pre-trained Language Models'', Proceedings of the Financial Technology and Natural Language Processing Workshop, 2019.} In settings where the downstream task is short-text polarity classification rather than long-form generation, bidirectional encoders retain an architectural advantage because they are directly optimised for discriminative supervision. This makes FinBERT a natural benchmark for the present chapter.

Decoder-only LLMs offer a different set of strengths. Parameter-efficient fine-tuning methods such as LoRA drastically reduce the memory and compute cost of adapting large generative models,\footnote{Edward J. Hu, Yelong Shen, Phillip Wallis, Zeyuan Allen-Zhu, Yuanzhi Li, Shean Wang, Lu Wang, and Weizhu Chen, ``LoRA: Low-Rank Adaptation of Large Language Models'', 2022.} while finance-specific open-source workflows such as FinGPT show how these methods can be applied to domain adaptation under realistic academic resource constraints.\footnote{Hongyang Yang, Xiao-Yang Liu, and Christina Dan Wang, ``FinGPT: Open-Source Financial Large Language Models'', arXiv:2306.06031, 2023.} These developments motivate testing generative backbones in this thesis, but they do not guarantee that a generative architecture will outperform a specialised encoder on a small supervised sentiment benchmark.

The transfer question is especially important in low-resource scenarios. Surveys of low-resource NLP emphasise that gains from pre-training and transfer learning become much less predictable when labelled data are sparse, domain shift is large, or the target language/domain combination is underrepresented in pre-training corpora.\footnote{Michael A. Hedderich, Lukas Lange, Heike Adel, Jannik Str\"otgen, and Dietrich Klakow, ``A Survey on Recent Approaches for Natural Language Processing in Low-Resource Scenarios'', NAACL-HLT 2021.} Malaysian financial news combines all three challenges: limited annotated data, local political-economic vocabulary, and a market context that differs materially from the US-centric corpora on which most financial NLP models are evaluated. The benchmark in this chapter therefore tests not only model capacity, but also the robustness of transfer to an emerging-market financial setting.
...
The three experiments converge on a coherent picture. Financial language models can extract meaningful sentiment signals from Malaysian financial news, but only when the corpus is sufficiently focused on the target firm. Sentiment-return relationships are contemporaneous for most stocks, limiting the practical forecasting horizon. Where lagged predictability exists (Tenaga, RHB), it is weak enough that sequence models show only modest directional improvements rather than large magnitude gains. The appropriate theoretical lens is semi-strong EMH: public information---including news sentiment---is rapidly priced in, but imperfect and delayed aggregation leaves narrow windows for above-chance prediction, particularly for stocks with lower analyst coverage or less liquid news flow.
...
\begin{enumerate}
    ...
    \item \textbf{Honest hybrid LSTM/GRU forecasting benchmark under correct methodological controls.} By targeting next-day \emph{returns} rather than absolute price levels, using strictly chronological data splits, fitting scalers only on training data, and reporting persistence-baseline comparisons, this thesis provides methodologically sound Experiment 3 results. These stand in contrast to published work that inadvertently reports look-ahead-biased metrics (e.g., same-day OHLCV features predicting same-day close), providing a reliable baseline for future Malaysian forecasting studies.
