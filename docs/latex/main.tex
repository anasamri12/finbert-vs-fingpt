...
\begin{enumerate}
    ...
    \item \textbf{Malay-language financial sentiment benchmark.} A 100-article benchmark dataset labelled by a domain-aware annotator, covering the three sentiment categories most commonly targeted in financial NLP (positive, neutral, negative). To the best of the author's knowledge, this is the first publicly documented evaluation benchmark for Malay-language financial sentiment, filling a gap in a language with limited NLP resources.

    \item \textbf{Systematic comparison of FinBERT and generative LLMs under Malaysian market conditions.} The benchmark evaluation demonstrates that fine-tuned FinBERT (macro F1=0.620) outperforms the strongest open zero-shot LLM in the main comparison, Meta-Llama-3.1-8B-Instruct (F1=0.458), as well as the TinyLlama-based FinGPT run2 model (F1=0.328). It also shows that LoRA fine-tuning with small labelled datasets actively harms Llama-8B performance, providing practitioners with concrete, empirically grounded guidance for model selection.

    \item \textbf{Empirical validation and quantification of signal dilution in financial news corpora.} The comparison between broad-corpus (null correlations) and company-filtered corpus (Pearson $r=0.32$--$0.34$, $p<10^{-18}$) results provides the clearest quantitative demonstration to date of the signal dilution effect in Malay-language financial news. This finding has direct practical implications for how sentiment signals should be constructed for downstream tasks.
...
\paragraph{Multilingual and cross-lingual model adaptation.}
The benchmark was constructed from Malay-language articles, but Malaysian financial news is routinely published in English, Simplified Chinese, and Bahasa Malaysia simultaneously. Multilingual encoder models (e.g., XLM-RoBERTa, mBERT) or cross-lingual transfer from English-domain FinBERT have not been evaluated on this corpus. Future work should assess whether cross-lingual transfer reduces the labelled-data requirement for Malay sentiment, and whether a mixed-language training corpus improves macro F1 for the neutral class specifically.
